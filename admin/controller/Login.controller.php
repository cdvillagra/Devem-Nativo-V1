    ...
    */
    ...
    public function enviarSenhaAction(){
        ...
        if($existe !== false){
            ...
            $conteudo = str_replace('[_TITULO_]', 'Redefinição de senha', $conteudo);
            $conteudo = str_replace('[_SAUDACAO_]', 'Olá '.$existe['auNome'], $conteudo);

            $texto = '<p>Uma redefinição de senha foi solicitada, acessando o link abaixo você poderá redefinir a senha e acessar o sistema.</p>';
            $texto .= '<p><a href="'.Url::baseAdmin('login/redefinirSenha/'.$existe['idUsuario'].'/'.$key).'">Redefina sua senha aqui</a></p>';
            $texto .= '<p>Caso não tenha solicitado essa redefinição, ignore esta mensagem.</p>';
